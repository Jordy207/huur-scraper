...
class TheHagueRealEstateScraper(BaseScraper):
    ...
    start_url = "https://www.thehaguerealestate.nl/aanbod"
    ...
    def search(self, max_retries: int = 2) -> list[Listing]:
        ...
        for link in soup.select("a[href*='/aanbod/']"):
            href = (link.get("href") or "").strip()
            if not href or href.endswith("/aanbod"):
                continue
            ...
            url = href if href.startswith("http") else f"https://www.thehaguerealestate.nl{href}"
            ...
            listings.append(
                Listing(
                    ...
                    bedrooms=None,
                    available_from=None,
                    raw_features={"selector": "a[href*='/aanbod/']"},
                )
            )
